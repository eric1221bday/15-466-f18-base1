...
// adapted from here: https://www.gamedev.net/forums/topic/552906-closest-point-on-triangle/
static glm::vec3 closest_point_on_triangle(const std::array<glm::vec3, 3> &triangle, const glm::vec3 &pos) {
...
WalkMesh::WalkPoint WalkMesh::start(glm::vec3 const &world_point) const {
  ...
  for (const glm::uvec3 &tri_indices : triangles) {
    std::array<glm::vec3, 3>
        tri_verts = {{vertices[tri_indices[0]], vertices[tri_indices[1]], vertices[tri_indices[2]]}};
    ...
    if (glm::distance(point, world_point) < min_dist) {
      ...
      closest.weights =
          barycentric(point, vertices[tri_indices[0]], vertices[tri_indices[1]], vertices[tri_indices[2]]);
    }
  }
